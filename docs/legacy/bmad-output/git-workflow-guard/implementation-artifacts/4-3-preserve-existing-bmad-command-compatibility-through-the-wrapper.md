...
이 문서의 mutating-tool guard 메시지 보존 요구는 `tech-spec-remove-layer-3-mutating-tool-guard.md`에 의해 더 이상 현재 런타임 계약이 아니다. 현재 계약은 BMAD workflow 중에도 `edit`/`write`/`patch`/`multiedit`를 `tool.execute.before`에서 허용하고, 변경 도구 실행 이후 `tool.execute.after`에서 workflow phase를 `"mutating"`으로 기록하는 것이다. Approval/startup-chain 대기 중 non-question tool 차단과 non-git workspace의 bash+git 차단은 별도 guard로 계속 유지된다.
