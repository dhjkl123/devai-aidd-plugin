...
export function buildFinalizationSentinelInstruction({ sessionID, commandName } = {}) {
  ...
  const instructionText = [
    ...
    "- If the user chooses Commit, run `git status` one more time immediately before the final commit decision.",
    "- If that final re-check still shows working-tree changes, you MUST perform the commit. Do not ask another approval question and do not skip the commit.",
    "- If that final re-check shows no working-tree changes left, do NOT attempt the commit. Finish normally and explain that there was nothing left to commit.",
    ...
    "- If the user chooses Commit, do not invent a different commit message, do not ask an extra approval question, and do not perform unrelated git commands.",
